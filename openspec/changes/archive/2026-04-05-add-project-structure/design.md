...
**Decision:** `ConnectionType` enum includes `postgres`, `mysql`, `mssql`, `sqlite`, `duckdb`, `motherduck`, and an `other` escape hatch. MSSQL is attached via DuckDB's community extension.

**Rationale:** DuckDB has official extensions for postgres and mysql, and a community extension for MSSQL. Keeping the type open-ended matches "essentially everything that DuckDB allows for."
...
- **MSSQL via community extension:** Less battle-tested than official extensions. Mitigated by: testing against real MSSQL instances during development.
