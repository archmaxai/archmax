...
`AGENT_API_KEY` is required for the AI-powered Semantic Model Builder and Testing Playground. The default provider is [OpenRouter](https://openrouter.ai); see the [Configuration](/reference/configuration/#ai-agent) reference for alternative providers.
...
<Aside type="caution">
Redis data is intentionally **not** persisted. The BullMQ queue is transient — incomplete jobs are retried on restart. If you need durable Redis data, configure an external Redis instance via `REDIS_URL` with its own persistence.
</Aside>

### Railway / Custom Volume Mounts
...
<Aside type="tip">
When using standalone Docker with embedded MongoDB, the single bind mount at `/data` captures all persistent state: project files, embedded MongoDB data, and the DuckDB extension cache. When using Docker Compose with external MongoDB, only project files (and DuckDB cache) live in the archmax container volume.
</Aside>
...
`UI_PASSWORD` is the source of truth for the admin password. On every container start the value is verified against the stored credential and, if it differs, the credential is replaced with the current `UI_PASSWORD`.

To rotate the admin password:

1. Update `UI_PASSWORD` in your environment (`.env`, compose file, or `docker run -e`) to the new value.
2. Restart the container (`docker compose restart archmax` or `docker restart archmax`).
3. Log in with the new password — the previous one no longer works.

<Aside type="caution">
Any password change made through the admin UI is overwritten on the next container restart unless `UI_PASSWORD` is updated to match. Treat `UI_PASSWORD` as authoritative.
</Aside>
...
<Aside type="caution">
Save your `ENCRYPTION_KEY` and keep it consistent across restarts and upgrades. If you change or lose it, stored credentials become unreadable and you will need to re-enter them.
</Aside>
