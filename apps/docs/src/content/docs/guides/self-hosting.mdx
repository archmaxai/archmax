...
| **Backups** | Each service has its own volume; easy to back up individually | Single `/home/archmax` bind mount captures project data, MongoDB (embedded), and DuckDB cache |
...
|------|----------|-------------|
| Project files | `/home/archmax/projects/` (archmax container) | Semantic model YAML files |
| Embedded MongoDB | `/home/archmax/mongodb/` (archmax container) | Database files (only when using embedded MongoDB) |
| DuckDB cache | `/home/archmax/.duckdb/` (archmax container) | DuckDB extension cache |
...
<Aside type="tip">
When using standalone Docker with embedded MongoDB, the single bind mount at `/home/archmax` captures the archmax user’s home: project files, embedded MongoDB data, and the DuckDB extension cache. When using Docker Compose with external MongoDB, only project files (and DuckDB cache) live in the archmax container volume.
</Aside>
