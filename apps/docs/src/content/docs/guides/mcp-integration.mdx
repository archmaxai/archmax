...
The view layer is a **logical** access control built on top of the federated DuckDB instance — it is not a process or kernel sandbox. See [Limits of the View Layer](/guides/data-federation#limits-of-the-view-layer) for what it does and does not protect against, and the operator checklist for how to combine it with upstream database privileges.
