...
archsem uses [DuckDB](https://duckdb.org/) as an in-process query engine that can federate queries across multiple database connections. Each project gets its own DuckDB instance with connections attached as named schemas.

## How It Works

When you add a connection to a project and mark it as active, archsem attaches it to the project's DuckDB instance using DuckDB's extension system:

| Database Type | DuckDB Extension |
|---------------|-----------------|
| PostgreSQL | `postgres` |
| MySQL | `mysql` |
