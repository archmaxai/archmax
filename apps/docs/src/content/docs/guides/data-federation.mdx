...
Under **Data Federation → Console** (`/<projectId>/connections/console`), you can run read-oriented SQL directly against the project's federated DuckDB instance (`SELECT`, `WITH`, `SHOW`, `DESCRIBE`, `EXPLAIN`). Unlike MCP `execute_query`, the console uses **raw connection slugs** (e.g. `shopify.public.orders`) — it does not go through scoped VIEWs.

The left **Setup commands** panel lists copyable examples:

- Pre-installed `INSTALL` / `LOAD` pairs (`postgres`, `mysql`, `sqlite`, `mssql FROM community`, `iceberg`, `httpfs`, `avro`)
- Per-connection **redacted** `ATTACH` strings (passwords and tokens shown as `********`)
- An example federation query when connections are active

Use **Install / Load** to run a single `INSTALL <name> [FROM community]` or `LOAD <name>` statement for additional community extensions. Extension state is held on the **API process** in-memory instance; the BullMQ worker maintains its own instance and will not see extensions installed from the console until its cache is rebuilt (e.g. after **Re-explore schemas** or a process restart).
