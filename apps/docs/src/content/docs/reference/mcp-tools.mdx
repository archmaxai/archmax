...
<Aside type="caution" title="Logical layer, not a process sandbox">
The validator + scoped-VIEW combination runs against the same DuckDB instance that holds every attached connection in the project. Modeller mistakes in `view_query`, co-tenancy across MCP tokens scoped to the same model, and DuckDB / extension privilege-escalation bugs are explicitly **out of scope** for this layer. See [Limits of the View Layer](/guides/data-federation#limits-of-the-view-layer) for the full list and the operator checklist.
</Aside>
