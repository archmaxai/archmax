...
|----------|---------|-----------------|
| `MONGODB_URI` | _(embedded)_ | **Optional.** When omitted, the entrypoint starts an embedded `mongod` at `mongodb://127.0.0.1:27017/archmax` with persistent data at `/home/archmax/mongodb`. Set this to use an external MongoDB instance. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ARCHMAX_DATA_DIR` | `/home/archmax/projects` | Base directory for semantic model YAML files. Normally not changed in Docker. |
...
|---------------|----------|-------------|-------|
| `/home/archmax/projects/` | Semantic model YAML files | Yes | Core application data |
| `/home/archmax/mongodb/` | Embedded MongoDB data files | Yes | Only used when `MONGODB_URI` is not set |
| `/home/archmax/.duckdb/` | DuckDB extension cache | Yes | Created when DuckDB loads extensions |
...
This bind mount maps the `archmax` user’s home directory inside the container to `~/.archmax` on the host, including project files (semantic model YAMLs), the DuckDB extension cache (`.duckdb/`), and embedded MongoDB data. When using Docker Compose with an external MongoDB service, the `mongodb/` directory is unused and MongoDB data is managed by the Compose `mongo` service volume.
...
```
1. Set ARCHMAX_DATA_DIR (default: /home/archmax/projects)
2. If MONGODB_URI is not set:
   ├── Create /home/archmax/mongodb (if missing)
   ├── Start mongod (fork, bind 127.0.0.1, data at /home/archmax/mongodb)
...
- Embedded MongoDB fails on corrupted data; delete the `/home/archmax/mongodb` directory and restart
