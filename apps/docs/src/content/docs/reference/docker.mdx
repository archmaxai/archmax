...
This mount captures both project data (semantic model YAML files) and embedded MongoDB data. When using Docker Compose with an external MongoDB service, the `mongodb/` directory is unused and MongoDB data is managed by the Compose `mongo` service volume.
