...
3. **Add a Connection**
   ...
   - **Type**: select your database type (Postgres, MySQL, MSSQL, SQLite, DuckDB)
...
4. **Build a Semantic Model**
   ...
   <Aside type="note">
   This step requires `AGENT_API_KEY` to be configured. If you see a warning banner instead of the chat interface, set the key in your `.env` file or Docker environment and restart. See the [Configuration](/reference/configuration/#ai-agent) reference for details.
   </Aside>
