...
End-to-end tests run Playwright against the full Docker image. They live in `apps/e2e/`.

To run them locally, start the application stack and then execute the tests:
...
The `docker-compose.ci.yml` starts the app image with local MongoDB and Redis, using test credentials (`admin` / `testpass123`). Playwright is configured to target `http://localhost:8080`.
