...
import { Project, McpToken, hashMcpToken } from "@semlayer/core/models/index";
...
import { registerSemlayerTools, type McpAuthContext, type McpToolContext } from "./semlayer-server";
...
app.all("/", async (c) => {
  ...
  if (isTestRoute) {
    tempDir = await mkdtemp(join(tmpdir(), "semlayer-test-build-"));
  ...
  await registerSemlayerTools(mcpServer, toolCtx);
